...
def insert_data(item):
    # insert data into db
    conn = sqlite3.connect(DB_FILENAME) # implicitly creates db if it doesn't exist already 
    cursor = conn.cursor()
...
@app.post("/generate") # change this to proper route
def get_text(prof: str=Form(), excuse: str=Form(), assgn: str=Form()):
    
    item = {"professor": prof, "excuse": excuse, "assignment": assgn}
